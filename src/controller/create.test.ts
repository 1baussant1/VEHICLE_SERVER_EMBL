...
describe('create vehicle controller', () => {
  ...
  test('creates a valid vehicle', async () => {
    // Modification de la structure de la requête pour faire len = 4
    const req = {
      body: {
        shortcode: 'abac',
        battery: 17,
    ...
    // When.
    await controller.handle(req as unknown as Request, resp as unknown as Response);
  ...
  test('rejects invalid shortcode', async () => {
    ...
    const req = {
      body: {
        shortcode: 'abacab',  // longueur incorrecte
        battery: 50,
        position: { latitude: 45, longitude: 45 } // <-- nécessaire
      },
    } as Request;
